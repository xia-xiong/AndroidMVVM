...
class ShapeConstraintLayout @JvmOverloads constructor(
    ...
    init {
        context.obtainStyledAttributes(attributeSet, R.styleable.ShapeConstraintLayout).apply {
            mShapeMode = getInt(R.styleable.ShapeConstraintLayout_svg_shapeMode, 0)
            mFillColor = getColor(R.styleable.ShapeConstraintLayout_svg_fillColor, 0XFFFFFFFF.toInt())
            mStrokeColor = getColor(R.styleable.ShapeConstraintLayout_svg_strokeColor, 0)
            mStrokeWidth = getDimensionPixelSize(R.styleable.ShapeConstraintLayout_svg_strokeWidth, 0)
            mCornerRadius = getDimensionPixelSize(R.styleable.ShapeConstraintLayout_svg_cornerRadius, 0)
            mCornerPosition = getInt(R.styleable.ShapeConstraintLayout_svg_cornerPosition, -1)
            mStartColor = getColor(R.styleable.ShapeConstraintLayout_svg_startColor, 0XFFFFFFFF.toInt())
            mEndColor = getColor(R.styleable.ShapeConstraintLayout_svg_endColor, 0XFFFFFFFF.toInt())
            mOrientation = getInt(R.styleable.ShapeConstraintLayout_svg_orientation, 0)
            mWithElevation = getBoolean(R.styleable.ShapeConstraintLayout_svg_withElevation, false)
            recycle()
        }
    }
